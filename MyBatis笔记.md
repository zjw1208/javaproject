...
- 坐标设计：GroupId一般分为多个段，这里我只说两段，第一段为域，第二段为公司名称。域又分为org、com、cn等等许多，其中org为非营利组织，com为商业组织。
...
需要注意的是，在xml配置文件中，url中的&符号需要转义成&。需要在&后添加amp;
...
![3](C:\Users\HP\Desktop\3.png)

## 创建Java类

建立表格t_user情况

![4](C:\Users\HP\Desktop\4.png)

new->class->com.zjw.mybatis.pojo.User

```java
package com.zjw.mybatis.pojo;
...
**右键  选中generate 进行构造 setget  toString**

## 创建mapper接口

>MyBatis中的mapper接口相当于以前的dao。但是区别在于，mapper仅仅是接口，我们不需要提供实现类
```java
package com.zjw.mybatis.mapper;  
...
- MyBatis映射文件存放的位置是**src/main/resources/mappers**目录（在下面创立一个directory）下
...
(<mapper namespace="com.zjw.mybatis.mapper.UserMapper"> )

namespace对应mapper的interface（接口）的全类名
...
<mapper namespace="com.zjw.mybatis.mapper.UserMapper">  
	<!--int insertUser();-->  
	<insert id="insertUser">  
		insert into t_user values(1,'张三','123',23,'男','123@qq.com')  
	</insert>  
</mapper>
```
...
- **在test  java 里面创建com.zjw.mybatis.test.MyBatisTestle类**
- **Alt +enter 可以自动加类**
```java
public class MyBatisTest {
    ...
    public void testInsertUser() throws IOException {
        ...
        //获取SqlSessionFactoryBuilder对象  为了建立工厂
...
```java
   //获取sqlSession，此时通过SqlSession对象所操作的sql都必须手动提交或回滚事务
	...
	//sqlSession.commit();
```
SqlSessionsqlSession=sqlSessionFactory.openSession(**b:true**);  //不太懂  **crtl P**
...
新建配置文件log4j.properties（文件名必须这个，放在resources目录）内容为：
...
3. 修改
  ...
  <!--int updateUser();-->
  <update id="updateUser">
      update t_user set username='小周' where id=11
  </update>
  ```

4. 查询一个实体类对象（关注下面的具体函数）
